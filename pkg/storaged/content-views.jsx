...
function append_row(client, rows, level, key, name, desc, tabs, job_object) {
    function menuitem(action) {
        if (action.title)
            ...
            return <DropdownSeparatorDeprecated className={action.only_narrow ? "show-only-when-narrow" : null} key="sep" />;
    }
...
const BlockContent = ({ client, block, allow_partitions }) => {
    ...
    return (
        <Card>
            <CardHeader actions={{ actions: <>{format_disk_btn}</>, hasNoOffset: false, className: undefined }}>
...
export class VGroup extends React.Component {
    ...
    render() {
        ...
        return (
            <Card>
                <CardHeader actions={{ actions: <>{new_volume_link}</>, hasNoOffset: false, className: undefined }}>
