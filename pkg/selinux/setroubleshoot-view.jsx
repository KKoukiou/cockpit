    ...
    render() {
        ...
        var fixEntries = this.props.details.pluginAnalysis.map(function(itm, itmIdx) {
            ...
            if (itm.doText && itm.doText.indexOf("\n") < 0)
                doElement = <TextArea isReadOnly defaultValue={itm.doText} />;
            ...
            if (itm.doText && itm.doText.indexOf("\n") >= 0) {
                ...
                doElement = parts.map(p => p[0] == "#" ? <TextArea isReadOnly key={p}>{p.substr(2)}</TextArea> : <span key={p}>{p}</span>);
            }

            return (
                <Card isCompact key={itm.analysisId + (itm.ifText || "") + (itm.doText || "")}>
                    <CardBody className="selinux-details">
                        <div>
                            <div>
